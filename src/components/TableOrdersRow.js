export default function TabelOrdersRow({ order, deleteOrder, showPopapEditOrder, changeEditValue, showPopapShowOrder }) {
  function handleEdit() {
    showPopapEditOrder();
    changeEditValue(order);
  }
  ...
  return (
    <tr className="table-order__row">
      <td>{ order.id }</td>
      <td>{ order.transportationDate }</td>
      <td>{ order.companyName }</td>
      <td>{ order.fullNameCarrier }</td>
      <td>{ order.phoneCarrier }</td>
      <td>{ order.comments }</td>
      <td>
        <a href={`https://ati.su/firms/${order.ati}/info`} target="_blanck">{ order.ati }</a>
      </td>
      <td>
        <div className="table-order__controls">
